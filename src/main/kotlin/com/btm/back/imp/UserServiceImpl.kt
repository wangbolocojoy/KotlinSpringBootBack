        ...
        return if (u != null) {
            ...
            if (code == null) {
                ...
                if (code == body.msgcode ?: "") {
                    ...
                    user.icon =
                        "https://myiosandroidkotlinapplication.oss-cn-chengdu.aliyuncs.com/home/picture/1/2020-07-31-00:19:34-0.png"
    ...
    override fun updateIcon(id: Int, uploadType: String, uploadFile: MultipartFile?): BaseResult {
        ...
        if (null != u) {
            return if (null != uploadFile) {
                ...
                val url = AliYunOssUtil.uploadToAliyun(
...
//            AliYunOssUtil.verificationCard()
    ...
    override fun uploadIdCard(userId: Int?, uploadType: String, uploadFile: MultipartFile?): BaseResult {
        ...
        if (model?.data == null) {
            ...
            if (authentication == null) {
                ...
                vo?.authentication =
                    !(authentica.NationalIdCard.isNullOrEmpty() || authentica.FrontIdCard.isNullOrEmpty())
                ...
                u.authentication =
                    !(authentication.NationalIdCard.isNullOrEmpty() || authentication.FrontIdCard.isNullOrEmpty())
                userrepository.save(u)
    ...
    override fun getIdCardInfo(body: ReqBody): BaseResult {
        val card = body.userId?.let { authenticationRespository.findByUserId(it) }
            ?: return BaseResult.FAIL("该用户没有实名认证信息")
    ...
    override fun addBackInfoPlist(body: ReqBody): BaseResult {
        val back =
            body.userId?.let { backdinfoplistRespository.findByUserId(it) } ?: return BaseResult.FAIL("用户不存在")
    ...
    override fun CreateVerifyScheme(body: ReqBody): BaseResult {
        ...
        val client = AsyncClient.builder()
            ...
            .build();
        if (body.Platform.isNullOrEmpty()) {
            return BaseResult.FAIL("请选择平台")
        }
        // Parameter settings for API request
        val getFusionAuthTokenRequest: GetFusionAuthTokenRequest
        if (body.Platform == "iOS") {
            if (body.BundleId.isNullOrEmpty()) {
                return BaseResult.FAIL("请输入BundleId")
            }
            getFusionAuthTokenRequest = GetFusionAuthTokenRequest.builder()
                .schemeCode("FA000000007250054023")
                .bundleId(body.BundleId)
                .platform(body.Platform)
                .durationSeconds(43200L) // Request-level configuration rewrite, can set Http request parameters, etc.
                // .requestConfiguration(RequestConfiguration.create().setHttpHeaders(new HttpHeaders()))
                .build()
        } else {
            if (body.PackageName.isNullOrEmpty() || body.PackageSign.isNullOrEmpty()) {
                return BaseResult.FAIL("请输入PackageName和PackageSign")
            }
            getFusionAuthTokenRequest = GetFusionAuthTokenRequest.builder()
                .schemeCode("FA000000007250054023")
                .packageName(body.PackageName)
                .packageSign(body.PackageSign)
                .platform(body.Platform)
                .durationSeconds(43200L) // Request-level configuration rewrite, can set Http request parameters, etc.
                // .requestConfiguration(RequestConfiguration.create().setHttpHeaders(new HttpHeaders()))
                .build()
        }
        val response = client.getFusionAuthToken(getFusionAuthTokenRequest)
        val resp = response.get()
        logger.info("resp----  ${resp.body}")
        client.close();
        return BaseResult.SUCCESS("成功", resp.body.model)

    }

    override fun getVerifyCodeandResister(body: ReqBody): BaseResult {
        val provider = StaticCredentialProvider.create(
            Credential.builder()
                .accessKeyId(System.getenv("ALIBABA_CLOUD_ACCESS_KEY_ID"))
                .accessKeySecret(System.getenv("ALIBABA_CLOUD_ACCESS_KEY_SECRET"))
